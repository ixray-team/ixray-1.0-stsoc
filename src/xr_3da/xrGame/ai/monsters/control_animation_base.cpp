...
void CControlAnimationBase::select_animation(bool anim_end)
{
	...
	// установить анимацию	
	string128	s1,s2;
	MotionID	cur_anim		= smart_cast<CKinematicsAnimated*>(m_object->Visual())->ID_Cycle_Safe(strconcat(sizeof(s2),s2,*anim_it->target_name,_itoa(index,s1,10)));
...
MotionID CControlAnimationBase::get_motion_id(EMotionAnim a, u32 index)
{
	...
	string128			s1,s2;
	return				(smart_cast<CKinematicsAnimated*>(m_object->Visual())->ID_Cycle_Safe(strconcat(sizeof(s2),s2,*anim_it->target_name,_itoa(index,s1,10))));
}
