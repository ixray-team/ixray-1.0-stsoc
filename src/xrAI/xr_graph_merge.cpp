...
#ifndef PRIQUEL
			CMemoryWriter					m_cross_table_;
			m_cross_table_.open_chunk(CROSS_TABLE_CHUNK_VERSION);
#endif // PRIQUEL
			m_cross_table_.w(&tCrossTableHeader,sizeof(tCrossTableHeader));
#ifndef PRIQUEL
			m_cross_table_.close_chunk();
			m_cross_table_.open_chunk(CROSS_TABLE_CHUNK_DATA);
#endif // PRIQUEL
			for (int i=0; i<(int)tCrossTableHeader.dwNodeCount; i++)
				m_cross_table_.w(&(tCrossTableUpdate[i]),sizeof(tCrossTableUpdate[i]));
#ifndef PRIQUEL
			m_cross_table_.close_chunk();
			strconcat				(sizeof(caFileName), caFileName,S,CROSS_TABLE_NAME);
			m_cross_table_.save_to	(caFileName);
#endif // PRIQUEL
		}
