...
#		define CHK_DX(expr)				do {static bool ignore_always = false; HRESULT hr_ = expr; if (!ignore_always && FAILED(hr_)) ::Debug.error(hr_,#expr,DEBUG_INFO,ignore_always);} while(0)
#	else // DEBUG
#		ifdef __BORLANDC__
#			define NODEFAULT
#		else
