...
namespace SmartDynamicCast {
	...
	template <typename T>
		struct search_base {
			...
			typedef typename 
				selector<
					std::is_same<Base,typename Head::Head>::value
				>::result result;
		};
	...
	struct conversion_sequence {
		...
		struct list_iterator {
			...
			struct helper {
				...
				typedef typename
					selector<
						std::is_same<Loki::NullType,search_result>::value
					>::result result;
			};
			...
			struct selector {
				...
				typedef typename _selector<!std::is_same<Loki::NullType,helper_result>::value>::result result;
			};
			...
			struct selector<false> {
				...
				struct _selector {
					...
					typedef typename _selector2<!std::is_same<Loki::NullType,helper_result>::value>::result result;
				};
		...
		struct selector {
			...
			typedef typename 
				_selector<
					!std::is_same<Loki::NullType,nearest>::value
				>::result result;
		};
...
#pragma todo("Dima to all : this smart_cast is not optimized!")
	...
	{
		return					(CHelper1<T1,T2>::smart_cast<std::is_base_of<T1,T2>::value || std::is_same<T1,T2>::value>(p));
	}
...
IC	T1	smart_cast(T2* p)
{
	...
	STATIC_CHECK				(std::is_void<std::remove_pointer<T1>::type>::value || std::is_polymorphic<std::remove_pointer<T1>::type>::value,Invalid_target_type_for_Dynamic_Cast);
	...
	return						(SmartDynamicCast::CHelper2<T2>::smart_cast<std::remove_pointer<T1>::type>(p));
}
...
#endif
	return						(*SmartDynamicCast::CHelper2<T2>::smart_cast<std::remove_reference<T1>::type>(&p));
}
